...
def createBIDS(projDir, pid, visitNum, sessionNum, runName = None):
    ...
    if op.isdir(fromDir + '/anatomical/'):
    ...
    if runName is None:
        ...
        for task in tasks: 
            ...
            if task == 'preprocessed':
                ...
                continue
            if op.exists(op.join(root, task)+ '/unnormalized'):
                for subRoot, dirs, files in os.walk(op.join(root, task)+ '/unnormalized'):
                    repTime = 0
                    for file in files:
                        if(file.endswith('.json')):
                            with open(op.join(subRoot, file)) as f:    
                                data = json.load(f)
                            repTime = data['RepetitionTime']
                            fileName = ('sub-%s_task-' % subStr) + task + '_bold.json'
                            shutil.copyfile(op.join(subRoot, file), toDir + '/func/' + fileName)  

                    for file in files:
                        if(file.endswith('.nii.gz')):
                            fileName = ('sub-%s_task-' % subStr) + task + '_bold.nii.gz'
                            if op.exists(op.join(subRoot, file)):
                                shutil.copyfile(op.join(subRoot, file), toDir + '/func/' + fileName)
    ...
    # Check if anat and func coordinates are too different, and if they are, fslreorient them
                ...
                for element in global_diff:
                    if float(element) > 25:
                        ...
                        if op.exists('%s/sub-%s/func/sub-%s_task-%s_bold.nii.gz'%(toDirRoot,subStr,subStr,task)) and op.exists('%s/sub-%s/func/sub-%s_task-%s_bold_orig.nii.gz'%(toDirRoot,subStr,subStr,task)):
    ...
    for root, dirs, files in os.walk(fromDir + '/fmri/' + runName + 'unnormalized'):
        ...
        for file in files:
            if(file.endswith('.json')):
                with open(op.join(root, file)) as f:    
                    data = json.load(f)
                repTime = data['RepetitionTime']
                fileName = ('sub-%s_' % subStr) + runName + '_bold.json'
                shutil.copyfile(op.join(root, file), toDir + '/func/' + fileName) 
                print("JSON copied") 
